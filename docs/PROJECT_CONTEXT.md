...
## 運用の要点
- ユーザーが「計画を立てるように」と依頼した場合は、`docs/plans/TEMPLATE.md` をベースに `docs/plans/{yyyy-mm-dd}_{HHMMSS}_{プラン名}.md` で計画書を新規作成する（JST日時）。
- `docs/reports/` に調査・分析・実行ログを新規作成する場合も、`docs/reports/{yyyy-mm-dd}_{HHMMSS}_{要約}.md` 形式を使う（JST日時）。
...
- `docs/PROJECT_CONTEXT.md` の更新履歴は本文内に持たず、`docs/history/{yyyy-mm-dd}_{HHMMSS}_{要約}.md` に記録する。
- 同一セッション内では `docs/history/` の同一履歴ファイルに追記して管理する。
...
- `docs/history/`: `docs/PROJECT_CONTEXT.md` 更新履歴のセッション別ログ
...
## 成果物配置メモ
- 計画書ではない調査・分析系のドキュメントは `docs/reports/{yyyy-mm-dd}_{HHMMSS}_{要約}.md` 形式で配置すると整理しやすい。
...
## 監査知見（AIエージェント基本開発要件, 2026-02-27 時点）
