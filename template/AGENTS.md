...
## 10.1 Subagent 運用
- 実装前のコード調査、影響範囲確認、テスト調査では、必要に応じて project-scoped custom agents を使う。
- 軽量な調査・探索・テスト確認は `gpt-5.4-mini` を指定した subagent に委譲する。
- subagent は原則 read-only とし、編集は親 agent が判断して行う。
- subagent を使う場合は、何を委譲したか、返ってきた要約、親 agent が採用した判断を `.codex/runs/<run_id>/REPORT.md` に記録する。
