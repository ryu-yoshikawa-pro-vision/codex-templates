...
現状の runner baseline は `template/scripts/codex-task.sh` と `template/scripts/codex-task.ps1` であり、`codex-safe` preflight を通した非対話実行、preset 選択、report JSON、JSONL log、run manifest、evaluation validation、scope check を担っている。

SDK runner はこれらの責務をすぐ置き換える前提ではなく、将来的な runner 改善候補として評価する。
...
今後 runner 機能を強化する場合、`codex-task` wrapper に機能を足し続けるべきか、SDK runner を experimental に評価すべきかの判断軸が必要である。

判断軸なしに個別機能を追加すると、wrapper 版と SDK 版で責務が重複し、run artifacts の source-of-truth が崩れる。
...
次を、既存 `codex-task` と同等以上の fidelity で記録できるかを評価する。
...
| Dangerous bypass prevention | dangerous bypass、危険 config、危険 add-dir を wrapper / policy で拒否する | bypass 不要で同等以上の防御を要検証 | Yes |
...
| JSONL log | preflight、exec、validation の event log を残す | structured event log を残せるか要検証 | Yes |
...
| Windows support | PowerShell wrapper を提供する | Windows native support を要検証 | Yes |
| Bash support | bash wrapper を提供する | Linux / WSL bash path を要検証 | Yes |
...
- SDK runner evaluation が不十分または不適切と判明した場合は、source repo example を継続停止し、`codex-task` baseline を維持する。
- 将来 SDK runner 実装PRが失敗した場合も、このADRに基づき experimental path を破棄できる。

## Follow-up work

- 実際にSDK runner candidateを評価する場合は、`examples/codex-sdk-runner/contract-checklist.md` を複製してevidenceを記録する。
- gap は harness improvement candidate として記録し、通常実装変更とは分離する。
- core template 採用は、別PRで adoption conditions を満たした evidence を示した後に判断する。
