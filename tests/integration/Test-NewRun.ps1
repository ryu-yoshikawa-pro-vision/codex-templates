...
try {
    ...
    if (@($manifest.hook_observations.log_paths).Count -ne 0 -or $manifest.hook_observations.blocking_event_count -ne 0 -or $manifest.hook_observations.safety_blocked_count -ne 0 -or $manifest.hook_observations.observation_error_count -ne 0) { throw "hook_observations defaults are out of contract" }
    if ($manifest.subagents.summary.total -ne 0 -or $manifest.subagents.summary.read_only -ne 0 -or $manifest.subagents.summary.writable -ne 0 -or $manifest.subagents.summary.scope_violations -ne 0 -or $manifest.subagents.summary.used_in_final_plan -ne 0) { throw "subagents defaults are out of contract" }
